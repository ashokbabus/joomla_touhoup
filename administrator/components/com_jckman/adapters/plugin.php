...
class JCKInstallerPlugin extends JObject
{
		...
		*/ 
		...
		if(!empty( $toolbarnames) && $row->icon)
		{
			...
			foreach($toolbarnames as $toolbarname)
			{
				...
				{ 	  
					JCKHelper::error('Failed to add '. $pname. 'plugin to  ' . $classname .' toolbar');
				} 	  
	...
	function uninstall($id)
	{
		...
		if (trim($row->name) == '') {
			JCKHelper::error( 'Plugin Uninstall: '.JText::_('Plugin field empty, cannot remove files'));
			return false;
		}
		...
		if (file_exists($manifestFile))
		{
			...
			if (!($xml =& JFactory::getXML($manifestFile))) {
				JCKHelper::error( 'Plugin Uninstall: '.JText::_('Could not load manifest file'));
				return false;
			}
			...
			foreach($toolbarnames as $toolbarname)
			{
				...
				if (!JFile::write($tmpfilename, $toolbarConfig->toString('PHP', array('class' => $classname . ' extends JCKToolbar'))))
				{
					JCKHelper::error('Failed to remove '. $row->name. 'plugin from  ' . $classname .' toolbar');
				}
			}
			...
			if (!JFile::write($cfgFile, $config->toString('PHP', array('class' => 'JCKToolbarPlugins extends JCKPlugins'))))
			{
				JCKHelper::error('Failed to remove '. $pname .' jckeditor plugin from config file');
			}

			$root =& $xml;

			if ($root->getName() != 'extension' && $root->getName() != 'install') {
				JCKHelper::error( 'Plugin Uninstall: '.JText::_('Invalid manifest file'));
				return false;
			}
			...
			JCKHelper::error( 'Plugin Uninstall: Manifest File invalid or not found. Plugin entry removed from database.');
